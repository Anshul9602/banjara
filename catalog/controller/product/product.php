...
class ControllerProductProduct extends Controller {
	...
	public function index() {
		...
		if ($product_info) {
			...
			foreach ($results as $result) {
				if ($result['image']) {
					$image = $this->model_tool_image->resize1($result['image'], '400', '530');
				} else {
					$image = $this->model_tool_image->resize1('placeholder.png', $this->config->get('theme_' . $this->config->get('config_theme') . '_image_related_width'), $this->config->get('theme_' . $this->config->get('config_theme') . '_image_related_height'));
				}
