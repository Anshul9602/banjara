...
class ControllerExtensionPaymentCcavenuepay extends Controller{
	public function index(){
		...
		if ($order_info)
		{
			...
			$redirect_url = $this->getBaseUrl('extension/payment/ccavenuepay/callback');
			$cancel_url = $this->getBaseUrl('extension/payment/ccavenuepay/callback');
	...
	public function callback(){
		...
		if ($order_info)
		{
			...
			if ($order_status == "Success") {
				if ($resOrderId == $order_info['order_id'] && $amount == round($order_info['total'], 2)) {
